...
refine_records <- function(db_conn = NULL, document_id,
  ...
  # Read interactions from database
  interactions <- get_existing_interactions(document_id, db_conn)

  if (is.logical(interactions) && is.na(interactions)) {
    message("No existing interactions found for refinement")
    return(list(
      status = "skipped",
      records_extracted = 0
    ))
  ...
  if (nrow(interactions) > 0) {
    ...
    interactions <- interactions[!interactions$human_edited & !interactions$rejected, ]
  }

  if (nrow(interactions) == 0) {
    message("No refinable interactions found (all protected or none exist)")
    return(list(
      status = "skipped",
      records_extracted = 0
    ))
  ...
  if (is.data.frame(refined_df) && nrow(refined_df) > 0) {
    ...
    tryCatch({
      ...
      message(glue::glue("Saved {nrow(refined_df)} refined records to database"))
      ...
      return(list(
        status = paste("Refinement succeeded but save failed:", e$message),
        records_extracted = 0
      ))
    })
  } else {
    message("No valid refined interactions returned")
    return(list(
      status = "completed",
      records_extracted = 0
    ))
