...
By default, ecoextract uses `anthropic/claude-sonnet-4-5` for extraction and `mistral` for OCR. To use different providers, pass the `model` or `ocr_provider` parameters to `process_documents()`. Supported LLM providers: Anthropic (`anthropic/`), Google Gemini (`google_gemini/`), OpenAI (`openai/`), Mistral (`mistral/`), Groq (`groq/`).

**Note:** OpenAI models require schemas to follow [OpenAI's structured output rules](https://platform.openai.com/docs/guides/structured-outputs/supported-schemas) -- specifically `"additionalProperties": false` on every object and all properties listed in `required`. Schemas generated by `init_ecoextract()` follow these rules by default. See the [Configuration Guide](https://n8layman.github.io/ecoextract/articles/configuration.html) for details.
