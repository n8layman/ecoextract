...
Skip logic uses a two-step check:
1. Check if status == "completed"
2. If yes, verify data actually exists in DB
3. If data missing, set status to error and re-run
...
| Extraction | `extraction_status` | `COUNT(*) > 0 FROM records WHERE document_id = ?` |
| Refinement | N/A (opt-in via `run_refinement`) | Records must exist to refine |

### Data/Status Desync Handling
...
# Helper function: check if step should run
...
# Step 3: Extraction
extraction_data_exists <- (record count > 0 for this document)
if (run_extraction AND should_run_step(extraction_status, extraction_data_exists, force_reprocess_extraction, document_id, metadata_ran)) {
  run Extraction
}
...
if (should_refine AND extraction_data_exists) {
  run Refinement
}
```
...
| Aspect | Current | Proposed |
|--------|---------|----------|
| Skip check | **NONE** - always runs | Status-first: check `extraction_status == "completed"` AND record count > 0 |
...
| Aspect | Current | Proposed |
|--------|---------|----------|
| `*_ran` tracking | **None** | Add `ocr_ran`, `metadata_ran`, `extraction_ran` flags |
...
4. **Fix extraction skip logic**:
   - `extract_records()` needs to check status and record count
