...
Create a `.env` file in the project root (**verify it’s in `.gitignore`
first**):
...
directory (via `.Rprofile`). Then run tests normally:
...
All test resources (temp databases, files, environment variables) are
cleaned up automatically using `withr`:
...
1.  **Am I testing functionality or content?** Tests should verify “can
    we save records?” not “does it find this specific species?”
2.  **Will this break if someone changes the schema?** If yes, make it
    schema-agnostic.
3.  **Am I testing LLM accuracy?** That’s outside the scope of package
    tests.
4.  **Does this need API calls?** If yes, add `skip_if()` guards for the
    required keys.
