...
By default, ecoextract uses `anthropic/claude-sonnet-5` for extraction
and `mistral` for OCR. To use different providers, pass the `model` or
`ocr_provider` parameters to
...
**Note:** OpenAI models require schemas to follow [OpenAI’s structured
output
rules](https://platform.openai.com/docs/guides/structured-outputs/supported-schemas)
– specifically `"additionalProperties": false` on every object and all
properties listed in `required`. ecoextract sets
`"additionalProperties": false` on every object before sending a schema
(except to Gemini, which rejects it); listing properties in `required`
is up to the schema. Schemas generated by
[`init_ecoextract()`](https://n8layman.github.io/ecoextract/reference/init_ecoextract.md)
follow these rules by default. See the [Configuration
Guide](https://n8layman.github.io/ecoextract/articles/configuration.html)
for details.
...
**Custom metadata:** The metadata step extracts bibliographic fields
(title, authors, year, DOI, …) by default. For corpora that are not
journal articles, add `ecoextract/metadata_schema.json` and
`ecoextract/metadata_prompt.md` (or pass `metadata_schema_file` /
`metadata_prompt_file` to
[`process_documents()`](https://n8layman.github.io/ecoextract/reference/process_documents.md))
to extract your own document-level fields, or set `run_metadata = FALSE`
to skip the step. New fields become columns in the `documents` table.
The schema’s `x-record-id-fields` names the fields that record IDs are
built from: metadata fields, `document_id`, or `file_name` (default:
`first_author_lastname` and `publication_year`, giving
`Smith_2020_1_r1`). List every metadata field in `required` with
nullable types, and keep nullable fields to 16 or fewer so the schema
works on older models such as Claude Sonnet 4.6.
...
When every model in `model` fails for a step, the step’s status
(e.g. `metadata_status` in the results and in the `documents` table)
lists each model’s error.

**Unexplained schema errors? Try a newer model before changing your
schema.** Older models enforce much tighter structured-output limits
than current ones, and report them as `Schema is too complex` or another
HTTP 400 that reads like a schema problem. The same schema can fail on
`anthropic/claude-sonnet-4-6` or `anthropic/claude-haiku-4-5` and pass
on `anthropic/claude-sonnet-5`. If every model in your fallback list
fails with the same schema error, check whether they are all older
models. See [Unexplained Schema
Errors](https://n8layman.github.io/ecoextract/articles/configuration.html#unexplained-schema-errors)
for the measured limits.

**Content refusals:** Some models refuse papers on sensitive topics
(e.g. select agents). Add a model from another provider to `model` as a
fallback (see [Model Fallback](#model-fallback)).

See the [Complete
Guide](https://n8layman.github.io/ecoextract/articles/ecoextract-workflow.html#troubleshooting)
for more.
...
Integration tests require API keys in a `.env` file and perform live
extractions against real APIs — they verify API response structure, not
content accuracy.

The `main` branch is protected. All changes must be submitted as a pull
request. PRs must pass the full CI suite before merging:

- `R CMD CHECK` with no errors or warnings
- Full test suite including integration tests (requires live API keys
  configured in CI)

This ensures every merge has been validated against real API behavior,
not just mocks.
