...
test_that("save_interactions_to_db inserts interactions", {
  db_path <- local_test_db()
  ...
  result <- save_interactions_to_db(db_path, doc_id, interactions, metadata)
...
test_that("save_interactions_to_db handles empty dataframe", {
  db_path <- local_test_db()
  

  empty_df <- data.frame()
  result <- save_interactions_to_db(db_path, 1, empty_df, list())

  expect_true(result)
})

test_that("save_interactions_to_db serializes JSON arrays", {
  db_path <- local_test_db()
  ...
  save_interactions_to_db(db_path, doc_id, interactions, list())
