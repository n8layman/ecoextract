...
test_that("ecoextract_info prints package information", {
  expect_output(ecoextract_info(), "ecoextract package")
  expect_output(ecoextract_info(), "Version: 0.1.0")
  expect_output(ecoextract_info(), "extract_interactions")
})
...
test_that("build_existing_interactions_context handles empty interactions", {
  context <- build_existing_interactions_context(NULL, NULL)

  expect_type(context, "character")
  expect_match(context, "No interactions")
})

test_that("build_existing_interactions_context handles zero-row dataframe", {
  empty_df <- data.frame()
  context <- build_existing_interactions_context(empty_df, NULL)

  expect_type(context, "character")
  expect_match(context, "No interactions")
})

test_that("build_existing_interactions_context formats interactions properly", {
  ...
  context <- build_existing_interactions_context(interactions, NULL)
...
test_that("build_existing_interactions_context handles missing organism names", {
  interactions <- data.frame(
...
test_that("build_existing_interactions_context generates fallback IDs", {
  interactions <- data.frame(
    bat_species_scientific_name = "Myotis lucifugus",
    bat_species_common_name = "Little Brown Bat",
