...
The metadata schema wraps its fields in a `publication_metadata` object and names the fields that form record IDs in `x-record-id-fields`:
...
Fields that are not already `documents` columns are added to the table automatically. Array and object fields are stored as JSON text. The default bibliographic columns stay in the table (empty for these documents), so `export_bibtex()` still works on journal-article databases.
